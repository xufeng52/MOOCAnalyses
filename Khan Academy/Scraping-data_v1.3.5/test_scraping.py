...
import json
import types
import get_origin_scratchpad_id
...
class user_detial(webapp2.RequestHandler):
	...
	session['message'] = ''
        ...
        self.response.write('<tr><th scope="col">Title</th><th scope="col">URL</th><th scope="col">Spinoff</th><th scope="col">Origin scratchpad id</th></tr></thead><tbody>')
        for i in range(len(results)):
            ...
            for j in range(3):
                ...
                self.response.write('</td>')
            self.response.write('<td>')
            parent_id = get_origin_scratchpad_id.find_parent(results[i][1])[0]
            if parent_id == "null":
                self.response.write("Original")
            else:
